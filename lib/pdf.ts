...
export async function generatePdf(
  ...
  const department = sheet.department || "Non precise";
  ...
  doc.setFontSize(19);
  doc.text("RAPPORT DE DONNEES", 16, 18);
  ...
  const metadata: string[][] = [
    ...
    ["Image source", sheet.source_filename || image?.name || "Non precisee"],
  ];
  ...
  if (sheet.rows.length) {
    autoTable(doc, {
      startY: lastY() + 8,
      head: [["Heure", "Quantite", "OK", "NOK", "Observation"]],
  ...
  for (let page = 1; page <= doc.getNumberOfPages(); page++) {
    ...
    doc.setFontSize(8);
    doc.text(
      "Rapport automatique - verifier les donnees avec la photo originale.",
      16,
      286,
    );
